...
export class Game {
  ...
  update(dt) {
    ...
    if (this.bossContactTimer > 0) {
      this.bossContactTimer = Math.max(0, this.bossContactTimer - dt);
    }
      ...
      if (circleCollision(enemy.x, enemy.y, enemy.radius, this.hero.x, this.hero.y, this.hero.radius)) {
        if (enemy.type.key === "BOSS") {
          if (this.bossContactTimer <= 0) {
            ...
            this.bossContactTimer = CONFIG.BOSS_CONTACT_COOLDOWN;
          }
        } else {
  ...
  updateWeapon(dt) {
    ...
    this.weaponPositions = this.computeWeaponPositions();
  }
  ...
  applyBombEffect() {
    ...
    this.bombFlash = 0.25;
    this.bombWave = 20;
    this.bombOrigin = { x: this.hero.x, y: this.hero.y };
    this.spawnBombParticles();
  }
  ...
  spawnBombParticles() {
    ...
    for (let i = 0; i < count; i += 1) {
      ...
      this.particles.push(new Particle(this.hero.x, this.hero.y, vx, vy, 0.7, size, "#ffd36f"));
  ...
  checkWeaponCollision(enemy) {
    const positions = this.weaponPositions.length ? this.weaponPositions : this.computeWeaponPositions();
  ...
  applyBossCollision(boss) {
    ...
    const overlap = this.hero.radius + boss.radius - dist;
    if (overlap > 0) {
      this.hero.x += (dx / dist) * overlap;
      this.hero.y += (dy / dist) * overlap;
    }
  ...
  render() {
    ...
    const weaponPositions = this.weaponPositions.length ? this.weaponPositions : this.computeWeaponPositions();
